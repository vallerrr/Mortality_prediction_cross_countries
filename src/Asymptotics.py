...
def asymptotics(model_set, start, end, step, domains):
    ...
    for model in model_set:
        ...
        for train_subset_size in np.arange(start, end, step):
            ...
            model_ = Models.Model_fixed_test_size(df, 0.3, domains, model, train_subset_size, polynomial,
                                                  y_colname='death')
...
for (m, n), subplot in np.ndenumerate(axis):
    seaborn.regplot(x=df_to_plot['train_subset_size'].astype(float), y=df_to_plot[columns[count]].astype(float),
                    color=color_blue, ax=axis[m, n])
