...
def data_reader():
    dfMort = pd.read_csv(os.path.join(os.getcwd(), "Python_hrsPsyMort_20190208.csv"), index_col=0)
...
if __name__ == '__main__':
    ...
    # specify dfs
    df_eval = pd.DataFrame(columns=['model', 'seed', 'fold_method', 'k', 'NOK', 'set', 'auc', 'f1', 'efron_r2', 'ffc_r2'])
    ...
    for seed in np.arange(seed_min_included, seed_max_not_included, 1):
        for k in k_list:
            ...
            if k == 1:
                ...
                df_eval.loc[len(df_eval), ] = [model, seed, 'NO fold splitting', k, NOK, 'test/validation',Eval['auc'], Eval['f1'],
                ...
                for cv_name in cv_dict.keys():
                    ...
                    for train_index, test_index in (cv.split(X, y) if cv_name == 'StratifiedKFold' else cv.split(X)):
                        ...
                        df_eval.loc[len(df_eval), ] = [model, seed, cv_name, k, NOK, 'test', Eval['auc'], Eval['f1'], Eval['efron_r2'], Eval['ffc_r2']]

                        # validation set
                        if 1 not in y_test:
                            continue
                        else:
                            pred, pred_prob = model.predict(X_test[domains]), model.predict_proba(X_test[domains])[:, 1]
                            Eval = seed_evaluate_metric(y_test, pred, pred_prob, y_train, X_test['sampWeight'])
                            df_eval.loc[len(df_eval), ] = [model, seed, 'NO fold splitting', k, NOK, 'validation',
                                                          Eval['auc'], Eval['f1'],
                                                          Eval['efron_r2'], Eval['ffc_r2']]
